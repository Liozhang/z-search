...
- 单条导入 / 批量导入到 Zotero 文献库（无 DOI 条目走标题→DOI 回退解析），
  导入落到当前选中的分类（无则 My Library 根）；开放获取 PDF 自动挂附件
  （设置可关）
- 「已在库」标记（DOI 与本地文库比对）防重复导入；「只看开放获取」筛选项
- 每张卡的引文钻取：点引用数看施引文献、点「参考文献」进 OpenAlex 引文
  列表，可直接导入
...
同一套离线徽章也可以出现在你文库的条目列表里（条目树新增「期刊指标」
列：IF、JCR/中科院分区、顶刊星标、预警/掠夺性标记）——免 key、零网络。
...
- 库内语义检索：标题/摘要元数据向量 + PDF 全文分块向量两路（笔记与
  PDF 注释一并索引），RRF 融合排序
...
- 工具栏放大镜按钮（设置可隐藏）、Ctrl/Cmd+Shift+K 快捷键，或工具菜单 ▸
  「打开搜索中心」打开搜索窗
